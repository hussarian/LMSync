...
export default function Header({ currentPage = "", userRole = "admin", userName = "사용자" }) {
      ...
      : userRole === "director" || userRole === "staff"
        ...
        : userRole === "instructor"
          ? [
              ...
              { href: "/academic", label: "학적부", key: "academic" },
              { href: "/exam", label: "시험", key: "exam" },
              { href: "/question-bank", label: "문제 은행", key: "question-bank" },
              { href: "/lecture-status", label: "강의 현황", key: "lecture-status" },
              { href: "/survey", label: "설문 평가", key: "survey" },
            ]
          : userRole === "student"
            ? [
                { href: "/my-courses", label: "내 수강 과목", key: "my-courses" },
                { href: "/my-grades", label: "시험 및 성적", key: "my-grades" },
              ]
            : []
  ...
  return (
    <header className="px-6 py-4 flex items-center justify-between" style={{ backgroundColor: "#2C3E50" }}>
      <div className="flex items-center space-x-8">
        <Link to="/dashboard" className="text-xl font-bold" style={{ color: "#1ABC9C" }}>
          LMSync
        </Link>
